...
for excel_file in excel_files:
    ...
    for i in range(top_ks.__len__()):
        ...
        if top_k == 10:
            ...
            k_ranges_random = k_ranges_anti_25
            l_ranges_opt_random = l_ranges_opt_anti_25
            l_ranges_max_random = l_ranges_max_anti_25
            l_ranges_uni_random = l_ranges_uni_anti_25
                        ...
                        if sheetname in wb.sheetnames:
                            ...
                            for m in range(types.__len__()):
                                ...
                                save_file_dir = save_file_path + str(dimension) + "D_top" + str(top_k) + "_budget_" + str(budget)\
                                                + "_" + type_name + "_" + card_excel[cc] + "_" + str(ss) + "/"
...
# base folder needs update as well
BASE_DIR = "../H2_ALSH/qhull_data/Synthetic/"
BASE_FOLDER = "../H2_ALSH/qhull_data/Synthetic/"
...
for ss in range(sample_range):
    for k in range(dimensions.__len__()):
        ...
        for cc in range(cardinality.__len__()):
            ...
            if any(x.startswith(str(dimension)) and x.endswith(ending_str) for x in os.listdir(parameter_path)):
                for ii in range(len(data_type)):
                    ...
                    f33.write("sample_index=" + str(ss) + "\n")
                    f33.write("test_str=random_4_1000000" + "\n")
                    ...
                    f33.write(
                        "# ------------------------------------------------------------------------------ \n")
                    f33.write("dPath=./raw_data/Synthetic/${test_str}/${datatype}_${d}_${cardinality}_sample_${sample_index}.txt \n")
                    f33.write(
                        "tsPath=./result/result_${datatype}_${d}D_${cardinality}_${sample_index} # path for the ground truth \n")
                    f33.write("qPath=./query/query_${d}D_${sample_index}.txt \n")
                    f33.write("oFolder=./result/result_${datatype}_${d}D_${cardinality}_${sample_index} \n")
...
print("about to work on generating bash files")

for ss in range(sample_range):
    for k in range(dimensions.__len__()):
        dimension = dimensions[k]
        for cc in range(cardinality.__len__()):
            total_bash_file = BASH_FILE_BASE_FOLDER + "run_bash_set_cur_" + str(dimension) + 'D_' + card_excel[cc] + '_' + str(ss) + '.sh'
            ...
            for m in range(types.__len__()):
                ...
                f10.write(
                    'sbatch ./' + "run_test_" + str(dimensions[k]) + "_" + str(
                        cardinality[cc]) + "_" + str(type_name) + "_" + str(ss) + ".sh sleep 2\n")

                bash_file = BASH_FILE_BASE_FOLDER + "run_test_" + str(dimensions[k]) \
                            + "_" + str(cardinality[cc]) + "_" + str(type_name) + "_" + str(ss) + ".sh"
                    ...
                    for i in range(top_ks.__len__()):
                        ...
                        if not os.path.exists(parameter_dir):
                            ...
                            BASH_FILE_FOLDER = BASH_FILE_BASE_FOLDER + "bash_set_" + str(dimension) + "D_top" + str(top_k) + \
                                               "_budget_" + str(budget) + "_" + type_name + "_" + str(cardinality[cc]) + "_" + str(ss) + "/"

                            TEMPORAL_RESULT = "../H2_ALSH/temp_result_" + str(dimension) + "D_top" + str(top_k) + "_budget_" + \
                                              str(budget) + "_" + type_name + "_" + str(cardinality[cc]) + "_" + str(ss) + "/"
                            ...
                            for ii in range(len(data_type)):
                                ...
                                folder_str = data_type[ii] + '_' + str(dimension) + '_' + str(cardinality[cc]) + '_' + str(ss)
                                # folder_str = data_type[ii] + '_' + str(dimension) + '_Sample_' + str(ss)
                                BASE_FOLDER = BASE_DIR + folder_str + '/'
                                ...
                                f3.write("sample_index=" + str(ss) + "\n")

                                temporalResult = TEMPORAL_RESULT_FOR_BASH + "run_test_${datatype}_${d}_${cardinality}_opt_${sample_index}"
                                overallResult = TEMPORAL_RESULT_FOR_BASH + "overall_run_test_${datatype}_${d}_${cardinality}_opt_${sample_index}"
                                sim_angle = TEMPORAL_RESULT_FOR_BASH + "sim_angle_${datatype}_${d}_${cardinality}_opt_${sample_index}"
                                ...
                                temporalResult = TEMPORAL_RESULT_FOR_BASH + "run_test_${datatype}_${d}_${cardinality}_max_${sample_index}"
                                overallResult = TEMPORAL_RESULT_FOR_BASH + "overall_run_test_${datatype}_${d}_${cardinality}_max_${sample_index}"
                                sim_angle = TEMPORAL_RESULT_FOR_BASH + "sim_angle_${datatype}_${d}_${cardinality}_max_${sample_index}"
                                f3.write("temporalResult=" + temporalResult + "\n")
                                f3.write("overallResult=" + overallResult + "\n")
                                f3.write("sim_angle=" + sim_angle + "\n")
                                f3.write("S=" + str(sim_threshold) + "\n")
                                ...
                                temporalResult = TEMPORAL_RESULT_FOR_BASH + "run_test_${datatype}_${d}_${cardinality}_uni_${sample_index}"
                                # overallResult = TEMPORAL_RESULT_FOR_BASH + "overall_run_test_${datatype}_${d}_${cardinality}_uni.txt"
                                overallResult = TEMPORAL_RESULT_FOR_BASH + "overall_run_test_${datatype}_${d}_${cardinality}_uni_${sample_index}"
                                sim_angle = TEMPORAL_RESULT_FOR_BASH + "sim_angle_${datatype}_${d}_${cardinality}_uni_${sample_index}"
                                f3.write("temporalResult=" + temporalResult + "\n")
                                f3.write("overallResult=" + overallResult + "\n")
                                f3.write("sim_angle=" + sim_angle + "\n")
                                f3.write("S=" + str(sim_threshold) + "\n")
