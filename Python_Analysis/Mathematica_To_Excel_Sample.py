...
def get_file_info_sample(full_file_name_):
    file_info_ = []
    file_name_ = full_file_name_.split('.txt')[0]
    temp_file_info_ = file_name_.split('_')
    dims_ = temp_file_info_[0]
    ...
    cards_ = re.sub('[.]', '', temp_file_info_[1])
    types_= temp_file_info_[3]
    sample_ = temp_file_info_[4]

    print('dim: ' + str(dims_) + ' cards: ' + str(cards_) + ' topks: ' + str(topks_) + ' types: ' + str(types_) + ' sample: ' + str(sample_))
    file_info_.append(str(dims_) + '_' + str(topks_) + '_' + str(cards_) + '_' + str(sample_))
    file_info_.append(str(types_))
    return file_info_
...
excel_file_name = excel_file_dir + str(dimension) + 'D_before_sample.xlsx'
...
for ss in range(sample_range):
    ...
    for file in os.listdir(text_file_path):
        if file.endswith(".txt"):
            ...
            file_info = get_file_info_sample(file)
