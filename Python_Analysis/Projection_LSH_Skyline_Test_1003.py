...
def computer_qhull_index(input_path, output_folder, aff_name, max_layers):
    ...
    for i in range(max_layers):
        output, output_cardinality, remain_data, remain_cardinality = find_skyline_bnl(data, cur_dimension,
                                                                                       cur_cardinality)
...
def select_dim(nums_, min_, max_):
    ...
    current_list = [0, 1, 4, 6, 7, 8, 11, 14, 16, 17, 18, 19, 21, 22, 26, 28, 31, 33, 34, 35, 36, 37, 40, 42, 45, 47,
                    50, 51, 52, 54, 55, 57, 59, 60, 63, 65, 69, 71, 75, 81, 82, 87, 89, 91, 92, 93, 94, 96, 98, 99, 3,
                    9, 10, 13, 15, 23, 25, 29, 30, 38, 39, 46, 48, 49, 53, 58, 61, 62, 68, 72, 73, 76, 79, 83, 88, 66,
                    5, 70, 41, 74, 80, 85, 86, 90, 95]
...
def save_transformed_data(data_type_, selected_index_, cardinality_, transform_list_, bin_index_, original_cardinality_,
                          index_list_):
    raw_data_ = []
    file_name_ = data_type_ + "_" + str(selected_index_) + "_" + str(original_cardinality_) + "_" + str(
        bin_index_) + ".txt"
    raw_data_.append(np.asarray(int(selected_index_)))
...
numss_ = [15, 20, 25, 50, 75]
dim_lists = []
dim_lists.append(range(0, 15))
dim_lists.append(range(0, 20))
dim_lists.append(range(0, 25))
dim_lists.append(range(0, 50))
dim_lists.append(range(0, 75))
...
for bb in range(bin_count):
    ...
    for ii in range(nums_):
        ...
        for kk in range(top_k):
            local_data_file = skyline_folder + data_type + "_" + str(cur_dim) + "_" + str(cardinality) + "_" + str(
                bb) + "_qhull_layer_" + str(kk)
...
print("current selected dim:  " + str(nums_) + " --  " + str(
    1.0 * sum(recall_list_transform) / recall_list_transform.__len__()))
