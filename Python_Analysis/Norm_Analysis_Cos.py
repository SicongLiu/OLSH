...
def cos_theta(K, L):
    if len(K) != len(L):
        return 0
    temp_dot_product = dot(K, L)
    temp_norm_K = np.linalg.norm(K)
    temp_norm_L = np.linalg.norm(L)
    return float(temp_dot_product)/float(temp_norm_K * temp_norm_L)
...
for ii in range(query_num):
    ...
    for jj in range(cardinality):
        ...
        temp_cos = cos_theta(cur_data, cur_query)
        cos_list.append(temp_cos)
cos_list = np.asarray(cos_list)

_ = plt.hist(cos_list, bins='auto')  # arguments are passed to np.histogram
print('Done')