    ...
    for j in range(len(Data_Types)):
        ...
        for jj in range(types.__len__()):
            ...
            l_anti_opt = post_optimization_opt(collision_probility, weight_anti, total_error, data_anti, k_anti, l_anti_opt,
                                           hash_used_anti_opt, hash_budget)
            l_corr_opt = post_optimization_opt(collision_probility, weight_anti, total_error, data_anti, k_corr, l_corr_opt,
                                           hash_used_corr_opt, hash_budget)
            l_random_opt = post_optimization_opt(collision_probility, weight_anti, total_error, data_anti, k_anti, l_random_opt,
                                           hash_used_random_opt, hash_budget)

            l_anti_uni = post_optimization_uni(data_anti, l_anti_uni, hash_used_anti_uni, hash_budget)
            l_corr_uni = post_optimization_uni(data_corr, l_corr_uni, hash_used_corr_uni, hash_budget)
            l_random_uni = post_optimization_uni(data_random, l_random_uni, hash_used_random_uni, hash_budget)
            ...
            for kk in range(len(l_anti_opt)):
                ...
                ws[cur_cell_random_opt] = l_random_opt[kk]
