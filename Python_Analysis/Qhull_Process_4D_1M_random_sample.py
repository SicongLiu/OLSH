...
if __name__ == '__main__':
    # command_bin_folder = '/home/sliu104/LSH_TopK_Saguaro/qhull/bin'
    # MY_DATA_FILE_PATH = '/home/sliu104/LSH_TopK_Saguaro/H2_ALSH/raw_data/Synthetic/'
    # OUTPUT_PATH = '/home/sliu104/LSH_TopK_Saguaro/H2_ALSH/qhull_data/Synthetic'
    ...
    OUTPUT_DIR = '/Users/sliu104/Desktop/StreamingTopK/H2_ALSH/qhull_data/Synthetic'
    ...
    for i in range(len(dimensions)):
        for j in range(len(cardinality)):
            for k in range(len(data_type)):
                for sr in range(sample_range):
                    data_file_name = MY_DATA_FILE_PATH + data_type[k] + str(dimensions[i]) + "_" + str(cardinality[j]) \
                                     + "_sample_" + str(sr) + ".txt"
                    sample_dir_name =  str(dimensions[i]) + '_Sample_' + str(sr)
                    OUTPUT_PATH = OUTPUT_DIR + '/' + data_type[k] + sample_dir_name
                    # new dir if necessary
