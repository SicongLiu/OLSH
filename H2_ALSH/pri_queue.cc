	...
	int max)							// max size
{
	num_ = 0;
	k_ = max;
	list_ = new Result_String[max + 1];
}

// -----------------------------------------------------------------------------
MinK_List_String::~MinK_List_String() 			// destructor
{
	if (list_ != NULL) {
		delete[] list_; list_ = NULL;
	}
...
// -----------------------------------------------------------------------------
bool MinK_List_String::isFull()			// is full?
{
	if (num_ >= k_) return true;
	else return false;
}
...
// -----------------------------------------------------------------------------
float MinK_List_String::insert(			// insert item
	float key,							// key of item
