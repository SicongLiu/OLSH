...
public:
	...
	void set_ith_id(int i, int new_id);
...
//  This structure is used for MIP search
// -----------------------------------------------------------------------------
class MinK_List_String {
public:
	MinK_List_String(int max);				// constructor (given max size)
	~MinK_List_String();					// destructor
	...
	// -------------------------------------------------------------------------
	std::string ith_id(int i) { return (i < num_ ? list_[i].id_ : ""); }
	...
	float insert(					// insert item
		float key,						// key of item
		std::string id);						// id of item

private:
	int k_;							// max numner of keys
	int num_;						// number of key current active
	Result_String *list_;					// the list itself
};
