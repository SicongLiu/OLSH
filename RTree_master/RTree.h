...
//		}
	...
	int MySearch(ELEMTYPE a_min[NUMDIMS], ELEMTYPE a_max[NUMDIMS], std::function<bool (const DATATYPE&)> callback) const;
...
RTREE_TEMPLATE
int RTREE_QUAL::MySearch(ELEMTYPE a_min[NUMDIMS], ELEMTYPE a_max[NUMDIMS], std::function<bool (const DATATYPE&)> callback) const
{
#ifdef _DEBUG
	for(int index=0; index<NUMDIMS; ++index)
	{
