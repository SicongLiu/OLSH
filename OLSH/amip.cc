		...
		const char  *output_folder)            // output folder
{
	...
	// fprintf(fp, "======================================================\n");    // flush the similarity value to file
	...
	for(int ii = 0 ; ii < threshold_conditions; ii++)
	{
		...
		for (int round = 0; round < max_round; ++round)
		{
			...
			for (int i = 0; i < qn; ++i)
			{
				...
				float temp_recall = calc_recall(top_k, (const Result *) R[i], list);
				recall[round] += temp_recall;


				// printf("TopK: %d, query index: %d, recall: %f .\n", kMIPs[round], i, temp_recall);
