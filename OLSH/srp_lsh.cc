	...
	int& hash_table_hit)
{
	...
	while(cur_candidates_size < top_k && cur_round_ < max_round_)
	{
		...
		// printf("looking into neighbour buckets, using threshold? :%d, cur_round: %d, cur_candidates_size: ?:%d \n", is_threshold, cur_round_, cur_candidates_size);
		++cur_round_;
		...
		for(int i = 0; i < L_; i++)
		{
			...
			// printf("At hash layer: %d, looking into neighbour buckets, cur_round: %d, cur_candidates_size: ?:%d \n", i, cur_round_, cur_candidates_size);
